...
if ($method == 'GET') {
    ...
    if ($action === 'all-listings') {
        $userId = verifyToken();
        // Check role
        $stmt = $conn->prepare("SELECT role, category FROM users WHERE id = ?");
        $stmt->execute([$userId]);
        $user = $stmt->fetch();
        
        if ($user && $user['role'] === 'staff') {
            $query = "SELECT * FROM businesses WHERE owner_id = ?";
            $params = [$userId];
            ...
            if (count($clientCategories) === 0) {
                $query = "SELECT * FROM businesses WHERE 1=0";
                $params = [];
            } else {
                $placeholders = implode(', ', array_fill(0, count($clientCategories), '?'));
                $query = "SELECT * FROM businesses WHERE category IN ($placeholders)";
                $params = $clientCategories;
            }
        } else {
